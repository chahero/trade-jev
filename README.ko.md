...
**2025년 1월 1~30일(UTC)**의 30일 실험을 완료하고 첫 7일 결과도 저장했습니다. Jev API **2,880회** 중 첫 **672회**가 7일 결과에 해당합니다. 두 결과는 동일 계좌의 중간·최종 기록이며 독립 실험이 아닙니다. 위 영상은 별도의 12시간 데모입니다.

초기 자금 10,000 USDT, 동일한 15분봉, 편도 수수료 0.10%·슬리피지 0.05%를 적용했습니다. Jev의 판단 지침은 실행 내내 동일하며 매 요청에 직전 마감 봉 96개를 전달했습니다.

| 전략 | 7일 수익률 | 7일 최대 낙폭 | 30일 수익률 | 30일 최대 낙폭 |
| --- | ---: | ---: | ---: | ---: |
| Jev | +1.56% | 3.07% | +2.81% | 5.11% |
| 규칙 전략 | +3.39% | 2.08% | -1.18% | 9.25% |
| 매수 후 보유 | +3.46% | 6.05% | +11.74% | 11.43% |
| 현금 | +0.00% | 0.00% | +0.00% | 0.00% |

이번 구간에서 Jev는 매수 후 보유보다 수익률과 낙폭이 모두 작았습니다. BTC 25%를 2,127회, 50%를 750회, 현금 100%를 3회 선택했고 BTC 75%·100%는 선택하지 않았습니다. 봉 종가 기준 평균 현금 비중은 **68.51%**였습니다. 낮은 투자 비중을 함께 고려해야 하며, 이 결과만으로 매매 타이밍 판단이 더 우수하다고 볼 수는 없습니다.

추가 모델 호출 없이 저장된 거래를 재계산해 체결·수수료·자산 곡선을 검증했습니다. 수익률에는 미청산 BTC와 모의 거래 비용이 반영되며 낙폭은 봉 종가 기준입니다. 한 과거 구간의 결과이고 고정 BTC 25%·50% 비중 전략과의 비교나 Jev 반복 실험은 하지 않았으므로, 다른 시장 국면이나 실시간 성과로 일반화할 수 없습니다.

[상세 보고서](docs/experiments/2025-01/REPORT.ko.md) · [7일 수치](docs/experiments/2025-01/day-7-summary.json) · [30일 수치](docs/experiments/2025-01/day-30-summary.json)
